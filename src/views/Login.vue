      ...
      <div class="login">
        ...
        <div class="btnSwitch">
          <div
            ...
            class="accountLogin switchBtn"
            :class="{active: loginType === 'normal'}"
          >
            <p>账户密码登录</p>
          </div>
          <div
            @click="loginType = 'face'"
            class="faceLogin switchBtn"
        ...
        >
          ...
          >
            ...
            >
              <p
                class="loginInput"
                :class="{focusShow:focusTarget === 'username'}"
              >
                <el-tooltip
                  v-model="tooltip.username.visible"
                  :manual="true"
                  :offset="0"
                  popper-class="errorTip"
                  effect="light"
                  content=""
                  placement="right"
                >
            ...
            >
              <p
                class="loginInput"
                :class="{focusShow:focusTarget === 'password'}"
              >
                <el-tooltip
                  v-model="tooltip.password.visible"
                  :manual="true"
                  :offset="0"
                  popper-class="errorTip"
                  effect="light"
            ...
            >
              <p
                class="captcha"
                :class="{focusShow:focusTarget === 'code'}"
              >
                <el-tooltip
                  v-model="tooltip.code.visible"
                  :manual="true"
                  :offset="0"
                  popper-class="errorTip codeErrorTip"
                  effect="light"
                  content=""
                  placement="right"
                >
                  <span slot="content">{{tooltip.code.message || 2234}}</span>
                  <input
                    @keyup.13="submitForm"
                    type="text"
                    v-model="user.code"
        ...
        <!-- 人脸登录框 -->
        <div
          class="faceMain mainMessage"
          v-else-if="loginType === 'face'"
        >
          <div
            class="faceBox"
            @mouseover="animationShow"
            @mouseout="animationHide"
          >
            <em></em>
...
export default {
  methods: {
    ...
    refreshCode() {
      const randomStr = randomCodes()
      this.code = `${codeURL}?${randomStr}`
    },
    ...
    showTooltip() {
      forEach(this.tooltip, (value, key) => {
        ...
        if (this.tooltip[key].visible) {
          this.tooltip[key].message = this.invalidFields[key][0].message
        }
      })
    },
    ...
    submitForm() {
      this.$refs.form.validate(async (valid, invalidFields) => {
        ...
        if (valid) {
          let data = {
            mobileNo: this.user.username,
            password: this.user.password,
            randomCode: this.user.code
          }
          Api.login
            .post(data, {
              loadingMaskTarget: '.loginBtn'
            })
            .then(res => {
              let data = res.data
  ...
  data() {
    const randomStr = randomCodes()
    let codeBaseURL = `${process.env.VUE_APP_API_HOST}/user/randomCode`
    return {
      invalidFields: {},
      focusTarget: '',
      loginType: 'normal',
...
body.login {
  background-image: url(~assets/images/login-banner.png), url(~assets/images/login-bg.png);
